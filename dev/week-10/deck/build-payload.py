...
Week 10 is ONE deck, unlike week 9's split pair, so there is no `--deck` flag.
...
It also refuses to build a payload that would be broken on arrival:
  ...
  * every `asset:<file>` referenced by the markdown must exist in `assets/`,
    and every file in `assets/` must be referenced (an unreferenced asset is
    usually a renamed figure — and unlike week 9, which shared one asset
    directory between two decks, here that is always a mistake);
  ...
  * every `sql-live` fence's `height=` must have a matching
    `--w10-editor-lines` rule in the stylesheet. That pair is what keeps a
    long query from being clipped mid-line while the slide still measures as
    fitting (see the deck README); a fence whose height has no rule would
    silently fall back to the platform's hardcoded `rows="8"`.
"""
...
CSS_PATH = HERE / "mbua512-week-10.css"
DECK_PATH = HERE / "deck.md"
DEFAULT_SLUG = "mbua512-week-10-ab-testing"
...
def main() -> int:
    ...
    if not DECK_PATH.exists():
        print(f"ERROR: {DECK_PATH.name} missing", file=sys.stderr)
        return 1

    md = DECK_PATH.read_text()
    ...
    for name in sorted(on_disk - referenced):
        problems.append(f"{name} is in assets/ but no slide references it")
    ...
    # Every fence's height needs its editor-lines rule, or the query is
    # clipped by the textarea's hardcoded rows="8" without registering as
    # overflow anywhere. `fit-sql-slides.py --write` keeps the two in step;
    # this is the check that they still are.
    fences = re.findall(r"```sql-live([^\n]*)\n", md)
    for i, info in enumerate(fences, start=1):
        m = re.search(r"height=(\d+)", info)
        if not m:
            problems.append(f"sql-live fence {i} has no height=")
            continue
        if f'[style*="{m.group(1)}px"]' not in css:
            problems.append(
                f"sql-live fence {i} is height={m.group(1)} but the stylesheet "
                f"has no --w10-editor-lines rule for it — run "
                f"`python3 fit-sql-slides.py --write`"
            )
