/**
 * Euclidean Distance Heatmap with Interactive Hierarchical Clustering Dendrogram
 * Euclidean distance: sqrt of sum of squared differences
 */

export async function initEuclideanHeatmap(containerId, options = {}) {
  const {
  ...
  console.log(`[Euclidean Heatmap] Initializing ${containerId} with ${dataFile}`);

  const container = document.getElementById(containerId);
  if (!container) {
    console.error(`[Euclidean Heatmap] Container ${containerId} not found`);
    return;
  }
  ...
  console.log(`[Euclidean Heatmap] Container dimensions: ${width}x${height} (offset: ${container.offsetWidth}x${container.offsetHeight}, client: ${container.clientWidth}x${container.clientHeight})`);

  // Load data
  try {
    const response = await fetch(dataFile);
    if (!response.ok) {
      console.error(`[Euclidean Heatmap] Failed to fetch ${dataFile}: ${response.status}`);
      container.innerHTML = `<div style="padding: 20px; color: red;">Failed to load data: ${response.status}</div>`;
      return;
    }
    const csvText = await response.text();
    console.log(`[Euclidean Heatmap] Data loaded, ${csvText.length} characters`);
    ...
    console.log(`[Euclidean Heatmap] Available: ${availableWidth.toFixed(0)}x${availableHeight.toFixed(0)}, Cell: ${cellSize.toFixed(1)}px, Heatmap: ${heatmapWidth.toFixed(0)}x${heatmapHeight.toFixed(0)}`);
    ...
    console.log(`[Euclidean Heatmap] Visualization complete for ${containerId}`);
  } catch (error) {
    console.error(`[Euclidean Heatmap] Error:`, error);
