/**
 * Cosine Similarity Network Graph Visualization
 * Shows European Big 5 leagues players connected by possession pattern similarity
 */

export async function initCosineNetwork(containerId, options = {}) {
  const {
    dataFile = 'data/distance-studies/cosine-player-network.json',
    width = 1200,
    height = 800,
    minEdgeWeight = 0.96,
    maxEdgeWeight = 0.999
  ...
  console.log(`[Cosine Network] Initializing ${containerId} with ${dataFile}`);

  const container = document.getElementById(containerId);
  if (!container) {
    console.error(`[Cosine Network] Container ${containerId} not found`);
    return;
  }
  ...
  try {
    ...
    if (!response.ok) {
      console.error(`[Cosine Network] Failed to fetch ${dataFile}: ${response.status}`);
      container.innerHTML = `<div style="padding: 20px; color: red;">Failed to load data: ${response.status}</div>`;
      return;
    }
    const data = await response.json();
    console.log(`[Cosine Network] Loaded ${data.nodes.length} nodes, ${data.edges.length} edges`);
  ...
  // Check if data has league or style attribute (for player or team networks)
  const hasLeague = data.nodes.length > 0 && data.nodes[0].league;
  const hasStyle = data.nodes.length > 0 && data.nodes[0].style;

  // League colors for player networks (Big 5 leagues)
  const leagueColors = {
  ...
  // Team style colors for team networks
  const styleColors = {
    'possession': '#16a085',    // Green - high possession
    'balanced': '#2980b9',      // Blue - balanced
    'counter': '#8e44ad',       // Purple - counter-attack
    'defensive': '#d35400'      // Orange - defensive
  };
  ...
  const label = sliderDiv.append('label')
    ...
    .style('font-size', '14px')
    .text('Minimum possession similarity: ');
  ...
  function updateGraph() {
    ...
    nodeMerge.select('circle')
      .attr('fill', d => {
        ...
        // Use style colors for team networks, league colors for player networks
        if (hasStyle) return styleColors[d.style] || '#999';
        if (hasLeague) return leagueColors[d.league] || '#999';
        return '#999';
      })
    ...
    nodeMerge.select('title')
      .text(d => {
        if (hasStyle && d.touches) {
          // Team network tooltip
          return `${d.id}\nStyle: ${d.style}\nGK: ${d.touches.GK.toFixed(1)} touches\nDEF: ${d.touches.DEF.toFixed(1)} touches\nMID: ${d.touches.MID.toFixed(1)} touches\nFWD: ${d.touches.FWD.toFixed(1)} touches\nTotal: ${d.touches.Total.toFixed(1)} touches/game`;
        } else if (hasLeague) {
          // Player network tooltip
          return `${d.id}\n${d.team}\n${d.league}\nAvg Possession: ${d.avgPossession} min`;
        }
        return d.id;
      });
