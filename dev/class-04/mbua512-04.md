...
**Query to write:** Find people whose birthday is today so we can email them (combine first and last names)
...
SELECT can include calculations. Some useful constructs: `concat` joins strings, `current_date` is the current date, `datediff` gives the difference between two dates in days, `floor` rounds a number down, `EXTRACT(MONTH from d)` gets the month part of a date (used later).

**Query to write:** Find people with a special birthday (e.g. 100) coming up (hint: need to calculate age, use FLOOR), order by closest birthday
...
- Find people with a birthday in August, September or October so we can email their planner (hint: use IN, can also use BETWEEN)
- Find people with missing or invalid email addresses (hint: use IS NULL AND ... NOT LIKE)
