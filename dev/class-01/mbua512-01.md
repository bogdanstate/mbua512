...
*Items 4–6 include group activities.*

---

# Introductions — who are we?

1. Professor **Michael Winikoff** — michael.winikoff@vuw.ac.nz (course coordinator & lecturer)
2. Dr **Bogdan State** — bogdan.state@vuw.ac.nz (lecturer)
3. **Steve Zhu** - s.zhu@vuw.ac.nz (senior tutor)
...
- **All deadlines are hard** — but extensions are possible if you're affected by
  circumstances outside your control; contact me (Michael) as soon as possible
- Late penalty: 7% for each (part) day, up to 5 days (applied hourly)
...
- We **discourage** any use of generative AI on these
...
- Inappropriate GenAI use risks a reduced mark (possibly even <span class="accent">0</span>)
...
![Entities and Attributes w:1180](img/entities-attributes.png)
...
1. Guided tour of user interface including demonstration of submitting an assignment and of drawing an ER diagram
2. Demonstration of creating tables (next slide)
3. Activity (rest of class)
...
> `PRIMARY KEY` indicates that `id` is the primary key - see next week 
...
- Varchar(length) - a string (`'Hello!'`) ["var" = variable length, "char" = character, note: use single quote!]
...
> *See the guide in Nuku*

_Note passwords and 2FA_
