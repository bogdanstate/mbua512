#!/usr/bin/env python3
"""Upload the week-09 deck to a Superset tier. RUNS INSIDE THE POD.
...
tier's web pod and executes it through `superset shell`:
    ...
    kubectl cp upload-deck.py  <ns>/<pod>:/tmp/w9-upload.py -c superset
    kubectl cp payload.json    <ns>/<pod>:/tmp/w9-payload.json -c superset
...
be truncated. So every outcome goes to /tmp/w9-result.json and the caller
